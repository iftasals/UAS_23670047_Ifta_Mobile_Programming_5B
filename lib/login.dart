...
class _LoginPageState extends State<LoginPage>
  ...
  Future<void> _checkIfAlreadyLoggedIn() async {
    try {
      ...
      if (mounted) {
        ...
        // Jika sudah login, langsung ke ProductListPage
        if (isLoggedIn) {
  ...
  //  FUNGSI LOGIN - VALIDASI DARI SHAREDPREFERENCES
  Future<void> _login() async {
    ...
    try {
      ...
      // Ambil list user yang sudah registrasi
      final usersJson = prefs.getString('users_list');

      if (usersJson == null) {
        throw Exception(
          'Tidak ada user terdaftar. Silakan registrasi terlebih dahulu.',
        );
      }
      ...
      final userFound = usersList.firstWhere(
      ...
      await prefs.setString('current_user_email', userFound['email']);
      await prefs.setString('current_user_name', userFound['name']);
      await prefs.setString('current_user_phone', userFound['phone']);

      if (!mounted) return;

      _showSnackBar(
        'Login berhasil! Selamat datang ${userFound['name']}',
        Colors.green,
      );
  ...
  // FUNGSI REGISTRASI - SIMPAN KE SHAREDPREFERENCES
  Future<void> _register() async {
    ...
    try {
      ...
      // Ambil list user yang sudah ada (jika ada)
      final usersJson = prefs.getString('users_list');
      List<dynamic> usersList = [];

      if (usersJson != null) {
        usersList = json.decode(usersJson);
      }
